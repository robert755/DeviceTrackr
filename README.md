...
2. **Database** - change your password and your ID in appsettings.json 

3. **User secrets** (from `backend\DeviceTrackr.Api`; never commit real secrets):
...
4. **API** — `dotnet run` in `backend\DeviceTrackr.Api` (see console for URL + Swagger).

5. **Frontend** — `cd frontend/devicetrackr-web` → `npm install` → `ng serve` → `http://localhost:4200`. Update `device-api.service.ts` if the API port changes.
